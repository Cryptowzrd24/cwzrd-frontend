    ...
    >
      ...
      >
        {newsData.map((item, index) => (
          ...
          <Box sx={item.name == "All News" ? {height:'40px',width:'90px', backgroundColor:'rgb(239,234,254)', borderRadius:'30px', display:'flex',alignItems:'center',justifyContent:'center', }: {}}>
            <Link key={item.id} sx={{ textDecoration: "none",textAlign:'center', fontSize:"14px",fontWeight:700,paddingInline:"5px",marginInline:'5px', color:`${ item.name == "All News" ? '#7248F7' : 'rgb(101,101,101)'}` }} href={item.link}>
              {item.name}
            </Link>
          </Box>
        ))}
